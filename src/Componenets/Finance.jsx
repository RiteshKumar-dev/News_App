...
const Finance = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://newsapi.org/v2/everything?q=finance&from=2024-06-25&sortBy=popularity&apiKey=${API_KEY}`
      );
  ...
  const storeData = async (articles) => {
    try {
      const response = await fetch(`${BASE_URL}/newsFinanceData.json`, {
        method: "POST",
        headers: {
  ...
  const fetchFirebaseData = async () => {
    try {
      ...
      localStorage.setItem("financedata", JSON.stringify(articles));
      setCards(articles);
